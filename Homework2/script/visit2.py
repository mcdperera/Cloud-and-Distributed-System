...
sites = []
#read the file, and only get the sites that this host should visit.
...
out_file = open(out_file_name,'w+')#csx?.dat
out_file.write('')
out_file.close()

protocols = ['http://', 'https://', 'http://www.', 'https://www.']
...
for i, site in enumerate(sites):
   ...
   for protocol in protocols:
      try:
         ...
         resp = requests.get(url, timeout=3, headers={'User-Agent':'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/61.0.3163.100 Safari/537.36 OPR/48.0.2685.35'});
