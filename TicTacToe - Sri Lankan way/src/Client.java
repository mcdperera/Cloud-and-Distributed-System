...
public class Client {
    ...
    class ListenFromServer extends Thread {
        ...
        private void playMoveGame(boolean isError) {
            if (isError) {
                display("Either wrong entry selection or already selected location or "
                        + "try to move other player's location");
            } else {
                display(Username + (isFirstUser ? " 'o'" : " 'x'") + ", enter your move "
                        + "(row[1-3] column[1-3])-(row[1-3] column[1-3])  format : ");
            ...
            String l1 = location.substring(0, 3);
            String l2 = location.substring(4, 7);

            if (isNotDuplicateLocation(l2)) {
                playMoveGame(true);
            } else if (isNotValidate(l2)) {
                playMoveGame(true);
            } else if (!isMovingOwnLocation(l1)) {
                playMoveGame(true);
            } else {

                GameMessage gameMessage = new GameMessage(GameMessage.PLAY_GAME,
                        Username + " moves " + l1 + " to " + l2, Username, l2, l1);
        ...
        // Check wether the user move location is previous 
        // moved by same user.
        private boolean isMovingOwnLocation(String location) {

            if (isFirstUser) {
                return firstPlayerLocations.contains(location);
            } else {
                return secondPlayerLocations.contains(location);
            }
        }
        ...
        private void setupSavedLocations(GameMessage message) {
            ...
            if (message.getRemoveLocation() != null && !message.getRemoveLocation().isEmpty()) {
                needToRemove = true;
            }
            ...
            if (message.getIsFirstUser()) {
                ...
                if (needToRemove) {
                    firstPlayerLocations.remove(message.getRemoveLocation());
                }
                ...
                if (needToRemove) {
                    secondPlayerLocations.remove(message.getRemoveLocation());
                }
